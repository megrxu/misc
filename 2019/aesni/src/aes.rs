...
pub fn key_expansion(key: &[u8], round_keys: &mut [AESstate]) {
...
pub fn add_round_key(state: &AESstate, round_key: &AESstate) -> AESstate {
    state.xor(round_key)
}
pub fn sub_bytes(state: &AESstate, sbox: &[u8]) -> AESstate {
    state.sub_sbox(sbox)
}
pub fn inv_sub_bytes(state: &AESstate, rsbox: &[u8]) -> AESstate {
    state.sub_sbox(rsbox)
}
pub fn shift_rows(state: &AESstate) -> AESstate {
    transpose(&transpose(state).lrot())
}
pub fn inv_shift_rows(state: &AESstate) -> AESstate {
    transpose(&transpose(state).rrot())
}
pub fn mix_columns(state: &AESstate) -> AESstate {
    [
        [0x02, 0x01, 0x01, 0x03],
        [0x03, 0x02, 0x01, 0x01],
        [0x01, 0x03, 0x02, 0x01],
...
pub fn inv_mix_columns(state: &AESstate) -> AESstate {
    [
        [0x0e, 0x09, 0x0d, 0x0b],
        [0x0b, 0x0e, 0x09, 0x0d],
        [0x0d, 0x0b, 0x0e, 0x09],
